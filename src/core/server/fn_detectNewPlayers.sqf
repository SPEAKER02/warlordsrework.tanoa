...
while { !BIS_WL_missionEnd } do {
	...
	private _value = 1;
	private _maxAi = (getMissionConfigValue ["WL2_MaxInfantrys", 8]);
	{
		if (_players < _x) then {
			_value = _value + 1;
		};
	...
	if (_value != _previousAIMax) then {
		missionNamespace setVariable ["BIS_WL_maxSubordinates_west", _maxAi, true];
		missionNamespace setVariable ["BIS_WL_maxSubordinates_east", _maxAi, true];
	};
