...
private _baseData =
	...
	BIS_WL_allSectors select {
		_x getVariable ["WL2_name", ""] in ["La Rachelle Aerodrom", "Bara Airstrip"];
	};
#else
...
0 spawn {
	...
	private _westSectors = BIS_WL_allSectors select {
		_x getVariable ["WL2_name", ""] in ["Blue Pear industrial port", "La Rochelle"];
	};
	private _eastSectors = BIS_WL_allSectors select {
		_x getVariable ["WL2_name", ""] in ["Lailai", "Cerebu"];
	};
	{
